...
const SkillBar: React.FC<SkillBarProps> = ({ skill, percentage }) => {
  ...
  return (
    <div ref={skillBarRef}>
      <div className="flex">
        <h2 className="text-[#282a29] font-FiraMedium">{skill}</h2>
      </div>
      <div className="flex items-center gap-5">
      <div className="w-1/2 max-sm:w-full bg-[#ffe6a7] border-2 border-[#282a29] rounded h-3 overflow-hidden">
        <div ref={barRef} className="bg-[#282a29] h-full"></div>
      </div>
        <span ref={numberRef} className="text-[#282a29] font-FiraMedium"></span>
      </div>
    </div>
  );
};
...
const About: React.FC = () => {
  return (
    <main className="bg-[#ffe6a7] max-sm:pb-20">
      <div className="flex max-sm:flex-col justify-center -mt-10 max-sm:-mt-5 px-14 items-center">
        <AboutPhoto/>
        <div className="max-sm:text-center max-sm:-mt-5">
          <h1 className="text-lg max-sm:text-sm text-[#282a29] font-FiraMedium">
          ...
          <h1 className="text-2xl text-[#282a29] font-FiraMedium mt-5 mb-2">My Skills:</h1>
